<?php
/**
 * This file is included from Vanilla with slight modifications.
 * The original can be found here: https://github.com/vanilla/vanilla/blob/Vanilla_2.2.1/library/core/class.passwordhash.php
 *
 * @copyright Copyright 2001 - 2013 by the original authors.
 * @license GNU GENERAL PUBLIC LICENSE Version 2
 */

namespace App\Externals\vanillaForums;
use \App\Externals\phpass\PasswordHash;
...
class Gdn_PasswordHash extends PasswordHash {
     ...
     * @return bool Returns **true** if the password matches the hash or **false** if it doesn't.
     */
    public function checkPassword($Password, $StoredHash, $Method = false, $Username = null, $Salt = null) {
        $Result = false;
        switch (strtolower($Method)) {
            case 'php':
                $Result = password_verify($password, $StoredHashd);
                break;
            case 'md5':
            ...
            case 'django':
                ...
                break;
            case 'drupal':
                require_once __Dir__ .'/../drupal/password.inc.php';
                $Result = \App\Externals\Drupal\user_check_password($Password, $StoredHash);
                break;
            case 'ipb':
...
#                $Salt = trim(substr($StoredHash, -$SaltLength, $SaltLength));
                ...
                break;
            case 'phpbb':
                include __Dir__ .'/../phpbb/phpbbhash.php';
